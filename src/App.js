...
const AREAS = ["Máquinas","Peso Livre","Cardio","Pilates","Funcional","Alongamento"];
...
// ═══ Firebase Auth (via REST API — sem SDK) ═══
...
async function fbResetPassword(email) {
  const res = await fetch(fbUrl("sendOobCode"), {
    method:"POST", headers:{"Content-Type":"application/json"},
...
async function apiGet(params) {
  if (API_URL==="COLE_SUA_URL_AQUI") return null;
  try {
...
const TABS = { REGISTRO:0, RESUMO:1, AOVIVO:2, PAINEL:3 };

// ═══════════════════════════════════════
//          LOGIN SCREEN
// ═══════════════════════════════════════
// Converte username pra email fake pro Firebase
...
function LoginScreen({ onLogin }) {
  ...
  const inp = {
    background:"#1a1a28", border:`1px solid ${C.border}`, borderRadius:10,
      ...
      <div style={{ width:"100%", maxWidth:380 }}>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:28 }}>
          ...
          <div style={{ fontSize:13, color:C.dim, marginBottom:24 }}>
            {mode==="login" ? "Digite seu usuário e senha" : "Escolha um usuário e senha"}
          </div>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <input value={username} onChange={e=>setUsername(e.target.value)} placeholder="ex: joao, leticia, admin..."
              style={inp} onKeyDown={e=>e.key==="Enter"&&handleSubmit()} autoFocus autoCapitalize="none" autoCorrect="off" />
          </div>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{
            width:"100%", padding:14, border:"none", borderRadius:10, cursor:loading?"wait":"pointer",
            background: loading ? C.dim : `linear-gradient(135deg,${C.accent},#a0cc00)`,
            color:C.bg, fontWeight:700, fontSize:15, fontFamily:"inherit", marginBottom:16,
            opacity: loading ? .6 : 1,
          }}>
        ...
        {FIREBASE_CONFIG.apiKey === "COLE_AQUI" && (
          <div style={{ marginTop:16, background:C.warningDim, border:`1px solid ${C.warning}44`, borderRadius:10, padding:"12px 16px", fontSize:12, color:C.warning, textAlign:"center" }}>
            ⚠ Firebase não configurado. Siga o guia para ativar o login.
          </div>
        )}
      </div>
    </div>
...
export default function AuraGym() {
  ...
  const [form, setForm] = useState({ nome:"",personal:"",area:"",obs:"" });
  ...
  // If Firebase not configured, skip login
  const skipAuth = FIREBASE_CONFIG.apiKey === "COLE_AQUI";

  useEffect(() => { if (user || skipAuth) { loadFromCloud(); const i = setInterval(loadFromCloud, 30000); return ()=>clearInterval(i); } }, [user, skipAuth]);

  const loadFromCloud = useCallback(async () => {
    const data = await apiGet({ action:"getToday", data:todayKey() });
    if (data?.records) {
      setRecords(data.records.map(r => ({ ...r, id:r.id||Date.now(), avaliacao:r.avaliacao?Number(r.avaliacao):null, saida:r.saida||null })));
      setConnected(true);
    }
  ...
  // Show login if Firebase is configured and user not logged in
  if (!skipAuth && !user) {
  ...
  const registrarEntrada = async () => {
    ...
    const record = { id:Date.now(), data:todayKey(), nome:form.nome.trim(), entrada:nowTime(), saida:null, personal:form.personal, area:form.area, obs:form.obs, avaliacao:null, lider:liderSala };
    setRecords(prev=>[...prev, record]);
    setForm({ nome:"",personal:"",area:"",obs:"" }); setShowForm(false);
  ...
  const isApiConfigured = API_URL !== "COLE_SUA_URL_AQUI";
          ...
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8 }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:10 }}>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:4 }}>
                ...
                <span style={{ fontSize:9,color:C.dim }}>{!isApiConfigured?"Offline":syncing?"Salvando...":connected?"Sync":"Erro"}</span>
              </div>
              ...
              {user && (
                <div style={{ display:"flex",alignItems:"center",gap:6,marginLeft:4 }}>
                  <div style={{ width:26,height:26,borderRadius:"50%",background:C.accent,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:C.bg }}>
                    {(user.displayName || fromFakeEmail(user.email) || "U")[0].toUpperCase()}
                  </div>
                  <span style={{ fontSize:11,color:C.text,fontWeight:500,maxWidth:80,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{user.displayName || fromFakeEmail(user.email)}</span>
                  <button onClick={handleLogout} style={{ background:"none",border:`1px solid ${C.border}`,borderRadius:5,padding:"4px 10px",color:C.dim,fontSize:10,cursor:"pointer",fontFamily:"inherit" }}>Sair</button>
                </div>
              )}
            </div>
          </div>
          <div style={{ display:"flex",gap:2,marginTop:10,background:"#090912",borderRadius:7,padding:2 }}>
            {tabItems.map(t2=>(<button key={t2.key} onClick={()=>setTab(t2.key)} style={{ flex:1,padding:"7px 4px",border:"none",borderRadius:5,cursor:"pointer",background:tab===t2.key?C.accent:"transparent",color:tab===t2.key?C.bg:C.dim,fontWeight:tab===t2.key?700:500,fontSize:11,fontFamily:"inherit" }}>{t2.label}</button>))}
          </div>
        </div>
      </div>

      {!isApiConfigured && (
        <div style={{ maxWidth:1080,margin:"12px auto 0",padding:"0 12px" }}>
          <div style={{ background:C.warningDim,border:`1px solid ${C.warning}44`,borderRadius:10,padding:"12px 16px",fontSize:13 }}>
            <strong style={{ color:C.warning }}>⚠ Modo offline</strong> — Dados na memória apenas. Siga o guia para conectar.
          </div>
        </div>
      )}
          ...
          {showForm && (
            <div style={{ background:C.card,border:`1px solid ${C.accent}33`,borderRadius:12,padding:18,marginBottom:18,boxShadow:`0 0 30px ${C.accent}11`,animation:"slideIn .2s ease" }}>
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
                ...
                <div style={{ gridColumn:"1/-1" }}><label style={{ fontSize:10,color:C.dim,textTransform:"uppercase",letterSpacing:1 }}>Observação</label><input value={form.obs} onChange={e=>setForm({...form,obs:e.target.value})} placeholder="Opcional..." style={{...inputStyle,marginTop:3}} /></div>
              </div>
              <button onClick={registrarEntrada} style={{ marginTop:14,width:"100%",padding:12,border:"none",borderRadius:8,background:`linear-gradient(135deg,${C.accent},#a0cc00)`,color:C.bg,fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"inherit" }}>⏱ Registrar Entrada — {nowTime()}</button>
            </div>
          )}
        ...
        {tab===TABS.RESUMO && (<div>
          ...
          {allNamed.length===0?(<div style={{ textAlign:"center",padding:50,color:C.dim }}><div style={{ fontSize:36,marginBottom:8 }}>📊</div><div>Nenhum registro</div></div>):(
            <div style={{ border:`1px solid ${C.border}`,borderRadius:10,overflow:"hidden",background:C.card }}><div style={{ overflowX:"auto" }}><div style={{ minWidth:totalW }}>
              ...
              {allNamed.map((r,idx)=>{const dur=timeDiffMin(r.entrada,r.saida);const tr=!r.saida;return(<div key={r.id} style={{ display:"flex",background:tr?C.successDim:(idx%2===0?C.rowOdd:C.rowEven) }}>
